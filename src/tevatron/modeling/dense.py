...
from transformers import AutoModel, PreTrainedModel, T5Model
...
class DenseModel(nn.Module):
    def __init__(
        ...
        super().__init__()   # self.training初始化nn.Module
    ...
    def encode_passage(self, psg):
        ...
        if self.model_args.character_query_encoder and not self.model_args.untie_encoder:
            ...
            decoder_input_ids = self.lm_p._shift_right(psg.input_ids)
            psg_out = self.lm_p(input_ids=psg.input_ids, decoder_input_ids=decoder_input_ids, return_dict=True)
    ...
    def encode_query(self, qry, typo_qry=None):
        if qry is None:
            return None, None
        if self.model_args.character_query_encoder:
            qry_out, q_hidden = self.lm_q(qry)     # qry size: batch * q_len * hidden_dim
        elif self.model_args.model_type == "t5":
            if not typo_qry:
                decoder_input_ids = self.lm_q._shift_right(qry.input_ids)
                qry_out = self.lm_q(input_ids=qry.input_ids, decoder_input_ids=decoder_input_ids, return_dict=True)
                q_hidden = qry_out.last_hidden_state
            else:
                decoder_input_ids = self.lm_q._shift_right(qry.input_ids)
                qry_out = self.lm_q(input_ids=typo_qry.input_ids, decoder_input_ids=decoder_input_ids, return_dict=True)
                q_hidden = qry_out.last_hidden_state
        else:
    ...
    def build(
        ...
        assert model_args.model_type in list(MODEL_TYPE_TO_MODEL.keys()), "model type should be 't5' or 'bert'"
        model_cls = MODEL_TYPE_TO_MODEL[model_args.model_type]
        # load local
        if os.path.isdir(model_args.model_name_or_path):
            if model_args.untie_encoder:
                ...
                if model_args.character_query_encoder:
                    ...
                    lm_p = CharacterBertModel.from_pretrained(_psg_model_path)
                else:
                    logger.info(f'loading query model weight from {_qry_model_path}')
                ...
                if model_args.character_query_encoder:
                    ...
                    lm_p = model_cls.from_pretrained(model_args.model_name_or_path, **hf_kwargs)
                lm_q = lm_p
        # load pre-trained
        else:
            if model_args.untie_encoder:   # not parameter sharing, default false
                lm_p = model_cls.from_pretrained(model_args.model_name_or_path, **hf_kwargs)
                if model_args.character_query_encoder:
