...
class ContrastiveLearningDenseTrainer(SelfTeachingDenseTrainer):
    pass

class RodrTrainingDenseTrainer(DenseTrainer):
    def compute_loss(self, model, inputs):
        if len(inputs) == 3:
            query, typo_query, passage = inputs
        else:
            raise NotImplementedError("rodr_training needed typos_query as input.")
        ...
        return model(query=query, passage=passage, typo_query=typo_query, original_reps=original_reps, current_step=self.state.global_step).loss


class QCDenseTrainer(DenseTrainer):
    def compute_loss(self, model, inputs):
        text, typo_text = inputs
        return model(typo_text=typo_text, text=text).loss
