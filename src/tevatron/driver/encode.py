...
from .train import MODEL_TYPE_TO_TOKENIZER
...
def main():
    ...
    # encode_in_path: 直接输入文件, 格式默认为 [query_id, query_text]
    if data_args.encode_is_qry:
        if data_args.encode_in_path:
            ...
            raise ValueError(
            f"Missing encoder query path, the enoder file should be formated: <query_id>, '\t', <query_text>")
    else:
        if data_args.encode_in_path:
            encode_dataset = datasets.load_dataset('json',
                                                data_files=data_args.encode_in_path[0],
